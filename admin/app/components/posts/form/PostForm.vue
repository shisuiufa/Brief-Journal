...
const formSchema = computed(() =>
  isEditMode.value ? updatePostSchema : createPostSchema,
);
...
const validateContent = async () => {
  try {
    ...
    // Validation errors are already handled by the form state.
  }
};
...
<template>
  ...
  >
      ...
      <UCard>
        <template #header>
          <div class="flex items-start justify-between gap-4">
            <div>
              <h2 class="text-base font-semibold">Post content</h2>

              <p class="mt-1 text-sm text-muted">
                {{
                  isEditMode
                    ? "Update the main information for the article."
                    : "Write the main information for the article."
                }}
              </p>
            </div>

            <UBadge :color="getPostStatusColor(state.status)" variant="soft">
    ...
    <aside class="space-y-6 xl:sticky xl:top-6 xl:self-start">
      <UCard>
        <template #header>
          <h2 class="text-base font-semibold">Publish settings</h2>
        </template>
        ...
        <div class="space-y-5">
          ...
          <div class="flex flex-col gap-3">
            ...
            <UButton to="/posts" color="neutral" variant="soft" block>
              Cancel
            </UButton>
          </div>
        </div>
      ...
      <UCard>
        <template #header>
          <h2 class="text-base font-semibold">Cover image</h2>
        </template>
